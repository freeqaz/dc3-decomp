#!/usr/bin/env python3
"""header_offset_audit_par.py -- DC3 port + parallel driver for the header
`// 0xHEX` offset-comment audit.

WHY A SECOND DRIVER
-------------------
``tools/header_offset_audit.py`` (ported from rb3-xenon) is strictly serial and
re-greps ``src/`` for a header once per class.  On this tree that is 848 TUs x
~14 s = ~3.3 h, plus tens of thousands of ``grep -rlP`` invocations.  Splitting
it into two phases makes it ~15 min and -- more importantly -- makes phase 2
**re-runnable for free**, so the audit rules can be corrected without paying for
848 compiles again.
...
It compares our header COMMENTS against OUR OWN COMPILER.  Every row is
