...
def generate_build_ninja(
    ...
    # The split-currency guard: which config/<version>/symbols.txt the TARGET
    # objects in build/<version>/obj/ were written from, and whether a split is
    # in flight right now. See scripts/verify_split_current.py for the
    # 341-function reproduction that motivates it.
    ...
    if build_config:
        ...
        ###
        # *** THE TARGET OBJECTS ARE AN UNDECLARED OUTPUT OF THE SPLIT. ***
        # `dtk xex split` writes build/<version>/obj/**.obj -- 2,223 files, the
        # ENTIRE target side of every diff -- and ninja knows about exactly one
        # of its outputs, build/<version>/config.json. Nothing names the
        # objects, so nothing can mark a report dirty when they move, and
        # `objdiff-cli report generate` run by hand (which tools/project.py's
        # report-cache comment notes is how most measurement here actually
        # happens) has no relationship to them at all.
        #
        # Today the chain that saves `ninja` is real but INCIDENTAL:
        # config.json -> build.ninja/objdiff.json -> report.json, and it holds
        # only because configure.py rewrites objdiff.json unconditionally. It is
        # one write_if_changed away from silently measuring pre-split objects,
        # and it does nothing for a direct measurement.
        #
        # What that costs, measured 2026-08-21 in one worktree with one
        # objdiff-cli (4.2.7 / 76c8da87e040) and the report cache COLD on both
        # runs: a report started 2 s into a split read 29,497 matched functions;
        # the same command on the same tree after the split finished read
        # 29,838. A 341-function gap discriminated only by timing, with no
        # error and no warning. (That gap was first blamed on a stale objdiff
        # report cache. The cache was tested directly -- mutating one byte of a
        # symbol name inside a target .obj turns its cache hit into a miss --
        # and exonerated. See scripts/verify_split_current.py.)
        #
        # So: an `always` edge, for the same reason as the alias-map check
        # above. Both of the ways this goes wrong are mtime-invisible -- a split
        # in flight leaves every object with a NEWER mtime than any report, and
        # a symbols.txt reverted by `git checkout` is newer than the objects it
        # disagrees with.
        ###
        ...
        # `always` makes this edge fire on demand, but firing is not the same as
        # measuring the right tree: baseline.json is generated by the same
        # `report` rule as report.json and so has the same two hidden inputs --
        # the rendered ICF-alias map and the target objects the split writes.
        # It listed neither, so `ninja baseline` could snapshot a pre-split
        # tree and every later `report changes` would be scored against it.
        n.build(
    ...
    n.rule(
        ...
        # 2,223 target objects are NOT declared ninja outputs, so nothing else
        # can tell a reader that they are mid-rewrite or that they came from a
        # different symbols.txt. --begin marks the stamp `running` BEFORE dtk
    ...
    n.build(
        outputs=str(split_stamp),
        rule="phony",
        inputs=str(build_config_path),
    )
    n.newline()
