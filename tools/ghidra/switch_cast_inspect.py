#!/usr/bin/env python3
"""
Switch/cast inspector over Ghidra decompiled C + raw PPC bytes.

NOTE: This tool does NOT export P-code (despite its former name pcode_inspect.py).
It is a switch-statement and sign/zero-cast inspector. For genuine Ghidra P-code use
the sibling tools/ghidra/pcode_export.py (HIGH or RAW P-code via in-process pyghidra).

This script analyzes:
- The annotated decompiled C output (which includes switch detection comments)
- Raw bytes at the function address for PowerPC instruction analysis (locally decoded)
- Cross-references for call target understanding

Usage:
    switch_cast_inspect.py "CharBones::PoseMeshes"
    switch_cast_inspect.py "0x82878b58"
    switch_cast_inspect.py "CharBones::PoseMeshes" --switches
    switch_cast_inspect.py "CharBones::PoseMeshes" --casts
