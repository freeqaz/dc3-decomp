...
## Current Status: Session 77 - WorldCrowd Billboard Rendering
**Goal**: Full visual quality — post-processing, lighting, particles, flares, lines, crowd all working
...
- **Session 77**: **WorldCrowd billboard rendering.** Implemented cached impostor RTT system for crowd rendering on native. Each crowd character archetype is rendered once to a per-character cached render target texture via `gImpostorCamera`, then instanced as billboard quads at all placement mesh positions. Fixed RTT clear color to use transparent alpha (0,0,0,0) so alpha cut correctly culls background pixels. Tested across all 5 crowd venues: dclive (101 instances, 5 crowds), dci (80 instances, 10 crowds), rollerrink (57 instances, 6 crowds), houseparty (76 instances, 7 crowds), streetside (76 instances, 6 crowds). All character references resolve correctly (0 null). Non-crowd venues (glitterati, throneroom) unaffected. Added WorldCrowd diagnostic in App.cpp (gated `#ifdef HX_NATIVE`).
- **Engine convergence (2026-05-27, Phase 0)**: **DC3 graduated to consuming the shared `../milo-native-engine` repo.** The engine-clean native code (gfx / audio / char / clean-platform, plus `Memory_Native`, `ThreadCall_Native`, and `Rnd_Wgpu` behind a `GameRenderHook` seam) was extracted out of `native/src/` into the sibling engine; all four DC3 native consumers (`dc3-native`, `milo-viewer`, `render-test`, `milo-tests`) now link `libmilo-engine.a` via `add_subdirectory`. The refactor was kept green throughout: **`milo-tests` maintained 371/371**. The engine compiles against DC3's matched-fork Milo headers via a consumer-injected context model (`MILO_ENGINE_DECOMP_INCLUDE_DIRS` / `_COMPAT_FLAGS` / `_PCH`). DC3 keeps its matched `src/system/` fork and per-decomp glue (`xdk_shims.cpp`, `msvc_compat.h`, etc.); the deferred-glue files awaiting HX_NATIVE-gating are `PlatformMgr_Native`, `RenderState_Native`, `Skeleton_Native`, `HttpServer`+`DebugPanel`, `MeshFilter`. Engine pin: `MILO_ENGINE_PIN = 9a58e86` in `native/CMakeLists.txt`. See `../milo-native-engine/docs/sessions/2026-05-27-phase-0-closure.md` and `rb3/docs/native/NATIVE_PORT_ROADMAP.md`.
...
6. **Venue rendering**: 272+ draw calls/frame during gameplay — venues with floor, walls, DJ booth, lighting rigs, fully-lit character, HUD overlays, particles, flares, lines, **crowd billboards** (up to 101 instances)
