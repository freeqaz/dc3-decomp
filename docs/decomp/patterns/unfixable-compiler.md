...
Wave 1 of the AT_LIMIT audit confirmed this across dozens of attempts: agents kept rewriting `(b + a)` vs `(a + b)`, `(x * y) - z` vs `z - (x * y)` (only legal for true commutative ops), etc., and the emitted code did not move in any case. Skip this rewrite — it is a wasted cycle.

> **Method correction (2026-08-31).** This row used to read *"the resulting `.obj` was byte-identical to the previous build in every case"* — the only claim in the pattern corpus phrased as a recompile-to-recompile **object-file** comparison. It cannot have been measured that way. Until `ee8902a22` every MSVC recompile changed every object's bytes (COFF `TimeDateStamp` + CodeView `S_OBJNAME` signature, both clock-derived), so a literal `sha256sum` A/B would have returned "differs" 980 times out of 989 and nobody would have written this row. What Wave 1 actually had was `run_objdiff` output — same score, same mismatch set. That is a normalized `.text` comparison, and it forgives register permutation and relocation names (see [relocation-names-are-unmetered.md](relocation-names-are-unmetered.md)), so it is strictly weaker than byte-identity.
>
> **The conclusion survives, and is now evidenced at the byte level for the first time.** Worktree at `ee8902a22`, full `ninja`, then per-target rebuilds of `LabelShrinkWrapper.obj` hashed with the two clock fields zeroed. Reordering both 3-term flat sums in `LabelShrinkWrapper::UpdateAndDrawWrapper` (`mRightBorder + mBoundsRight + mBoundsLeft` → `mBoundsLeft + mBoundsRight + mRightBorder`, and the `maxZ` sum likewise) left the object at masked `0aec645977d018bb` — **identical to the unedited baseline**, while the *raw* hash moved `b4493fa8…` → `f2ab09cf…` on pure clock noise. Sabotage control, same function, same rig: flipping one `+` to `-` moved the masked hash to `35eb83dd63af05d0`, and restoring the source returned it to `0aec645977d018bb`. So the instrument can see a change to this expression, and flat-sum term order does not produce one. The actionable lever for a `COMMUTATIVE_OP_ORDER` mismatch is **upstream**: reorder the variables that feed the op so the register allocator assigns the operands to the registers the target chose. See [Register Allocation](#register-allocation) — the swap is a regalloc artifact, not an expression-evaluator artifact.
