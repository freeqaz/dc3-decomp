...
- [fixable-inline-boundary.md: Inline-Level Counting](fixable-inline-boundary.md#inline-level-counting-via-the-parameter-home-area) — the dead temp-slot lever (the "parameter home area" name is a misnomer, see the [2026-08-06 correction](fixable-inline-boundary.md#correction-2026-08-06--not-an-abi-property-and-not-the-home-area)); a common non-liveness cause behind a swap cascade, but strictly qualifying sites are rare — pre-qualify on EH state + computed sub-object `this` + ≥2 uses of `this` before spending a build
