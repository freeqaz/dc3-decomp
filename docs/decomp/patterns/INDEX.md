...
- **2026-09-13 — "a comment shifts `__LINE__` in every MILO assert below it" is FALSE here; the
  ...
  **4 of 1,188** `.cpp`/`.c` files under `src/` contain a literal `__LINE__`, and only
  ...
  hash with **zero** codegen change — compare section multisets with relocations resolved to
  symbol *names*, never indices.
...
| Inline-Level Counting (dead temp-slot store; *not* the parameter home area) | 0 to +7% (2 ctors to 100%), but **only 14 fns in the whole binary** qualify strictly | low overall; high for ctors whose loose scalar fields are really an aggregate. Needs EH state + computed sub-object `this` + ≥2 uses of `this` | [fixable-inline-boundary.md](fixable-inline-boundary.md#inline-level-counting-via-the-parameter-home-area) ([2026-08-06 correction](fixable-inline-boundary.md#correction-2026-08-06--not-an-abi-property-and-not-the-home-area)) |
| `throw()` on a callee MSVC cannot prove nothrow (removes the whole EH state: r31 frame + dead `this` home store + one extra callee-saved GPR) | +18.7pp and +30pp on the two known instances (both to 100.0%) | **EXHAUSTED — 0 full-signature rows left binary-wide** out of 30,524 target/base pairs; 1 partial, worked to 100.0%. Read the section before adding one: it is an exception *specification*, clang enforces it, MSVC does not | [fixable-inline-boundary.md](fixable-inline-boundary.md#condition-1-is-a-source-lever--the-callee-msvc-cannot-prove-nothrow) |
...
| `has_prologue_mismatch` | **Was identically 0; repaired 2026-08-19 (221 rows).** The detector was fine — `sync_objdiff` ran objdiff with `functionRelocDiffs=none`, which masks the relocation diffs it reads. Same fault hid `has_linker_merged` (1,310), `has_scope_counter_mismatch` (81) and `has_makestring_mismatch` (63). The "PROLOGUE_MISMATCH is a positive indicator" heuristic **is** now applicable from the DB, but only after `python3 scripts/backfill_reloc_patterns.py --apply`. **Triaged the same day** — [2026-08-19-reloc-pattern-flag-triage.md](../../analysis/2026-08-19-reloc-pattern-flag-triage.md): **0 of 218 rows carry the prologue as their only pattern** and 93.1 % co-occur with `REGISTER_SWAP`, so use it as a *filter inside* the liveness lane (`has_prologue_mismatch=1 AND has_register_swap=1`), never as a lane of its own. |
| `has_linker_merged` | **Not an ICF detector, despite the name.** On the 1,052 rows that reproduce, 20.7 % of the "merged functions" it names are `__savegprlr_N`/`__restgprlr_N` helpers and 6.3 % are `MakeString<>` instantiations — `has_prologue_mismatch` and `has_makestring_mismatch` are *strict subsets* of it. Only **1.9 %** name a symbol that is ICF by construction. 65 % of the bucket vanishes under the graded `functionRelocDiffs=name_check` ruler. Do not open a lane on it. |
| `has_makestring_mismatch` | **The one bucket of the three with real bugs (2026-08-19).** 77.8 % artifact; the 14 survivors are wrong-callee bugs that `match_percent_normalized` *cannot see*, because a relocation-name diff is `arg_diff_score`. Decide each row against `ham_xbox_r.map`'s `FormatString::operator<<` fold classes — note `operator<<(int)` is its **own** class at `0x827ca420`, separate from the unsigned/long/`void*` family at `0x827ca848`. Use the payload's **instruction index** to attribute the site, and change a variable's declared type rather than casting at the call site. |
