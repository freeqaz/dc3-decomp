...
The Dance Central 3 decompilation targets a **debug build** (no link-time optimization) for Xbox 360 (PowerPC). The compiler is MSVC for Xbox 360, targeting the PowerPC 970 (Xenon) architecture. When recompiling C++ source that produces semantically identical code, the compiler frequently assigns different registers to the same variables — causing ~607 functions to plateau at 90-97% match with no source-level fix.
