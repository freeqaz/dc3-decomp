...
============================================================
  ...
  [XEX total — XDK-diluted headline]
    Matched code (raw bytes):  44.24%  (5,033,788 / 11,379,344 bytes)
    Matched fns  (fuzzy==100): 60.99%  (29,526 / 48,412)
    Matched fns  (norm==100):  61.42%  (29,733 / 48,412)
    Complete units:            21.12%  (434 / 2055)

  [Authorable — CANONICAL HEADLINE]
    Matched code (raw bytes):  79.27%  (5,032,900 / 6,349,076 bytes)
    Matched fns  (fuzzy==100): 91.50%  (29,512 / 32,252)
 ** Matched fns  (norm==100):  92.15%  (29,719 / 32,252)  <-- CANONICAL
    Complete units:            44.88%  (434 / 967)

  [Remaining authorable work  (normalized < 100 %)]
    Functions:  2,533
    Bytes:      1.20 MB (1,204,892 bytes)

e; counts vendor bytes in denominator |
| **Authorable fuzzy** | 79.27 % | Raw matched-code bytes over authorable-only total; best apples-to-apples byte signal |
| **Authorable normalized %** ✅ | **92.15 %** | **CANONICAL.** Functions where `match_percent_normalized == 100` over authorable total. Forgives register permutation / benign reloc-addend, but NOT wrong constants, offsets, or vtable slots |
...
| Matched code (raw bytes) | 5,032,900 bytes → **79.27 %** |
| Matched fns (fuzzy == 100) | 29,512 / 32,252 → 91.50 % |
| **Matched fns (normalized == 100)** | **29,719 / 32,252 → 92.15 %** |
| Complete units (all fns norm==100) | 434 / 967 → 44.88 % |
| Remaining fns (norm < 100) | 2,533 |
| Remaining bytes (norm < 100) | 1,204,892 bytes (1.20 MB) |
...
| Matched code (raw bytes) | 5,033,788 bytes → 44.24 % |
| Matched fns (fuzzy == 100) | 29,526 / 48,412 → 60.99 % |
| Matched fns (normalized == 100) | 29,733 / 48,412 → 61.42 % |
