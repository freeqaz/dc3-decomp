...
============================================================
  ...
  [XEX total — XDK-diluted headline]
    Matched code (raw bytes):  44.25%  (5,035,176 / 11,379,344 bytes)
    Matched fns  (fuzzy==100): 61.01%  (29,538 / 48,412)
    Matched fns  (norm==100):  61.44%  (29,746 / 48,412)
    Complete units:            21.12%  (434 / 2055)

  [Authorable — CANONICAL HEADLINE]
    Matched code (raw bytes):  79.29%  (5,034,288 / 6,349,076 bytes)
    Matched fns  (fuzzy==100): 91.54%  (29,524 / 32,252)
 ** Matched fns  (norm==100):  92.19%  (29,732 / 32,252)  <-- CANONICAL
    Complete units:            44.88%  (434 / 967)

  [Remaining authorable work  (normalized < 100 %)]
    Functions:  2,520
    Bytes:      1.20 MB (1,202,524 bytes)
...
| **Authorable normalized %** ✅ | **92.19 %** | **CANONICAL.** Functions where `match_percent_normalized == 100` over authorable total. Forgives register permutation / benign reloc-addend, but NOT wrong constants, offsets, or vtable slots |
...
| Matched code (raw bytes) | 5,034,288 bytes → **79.29 %** |
| Matched fns (fuzzy == 100) | 29,524 / 32,252 → 91.54 % |
| **Matched fns (normalized == 100)** | **29,732 / 32,252 → 92.19 %** |
| Complete units (all fns norm==100) | 434 / 967 → 44.88 % |
| Remaining fns (norm < 100) | 2,520 |
| Remaining bytes (norm < 100) | 1,202,524 bytes (1.20 MB) |
...
| Matched code (raw bytes) | 5,035,176 bytes → 44.25 % |
| Matched fns (fuzzy == 100) | 29,538 / 48,412 → 61.01 % |
| Matched fns (normalized == 100) | 29,746 / 48,412 → 61.44 % |
