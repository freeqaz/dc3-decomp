...
============================================================
  ...
  [XEX total — XDK-diluted headline]
    Matched code (raw bytes):  44.32%  (5,043,036 / 11,379,344 bytes)
    Matched fns  (fuzzy==100): 61.08%  (29,568 / 48,412)
    Matched fns  (norm==100):  61.51%  (29,776 / 48,412)
    Complete units:            21.17%  (435 / 2055)

  [Authorable — CANONICAL HEADLINE]
    Matched code (raw bytes):  79.42%  (5,042,148 / 6,349,076 bytes)
    Matched fns  (fuzzy==100): 91.63%  (29,554 / 32,252)
 ** Matched fns  (norm==100):  92.28%  (29,762 / 32,252)  <-- CANONICAL
    Complete units:            44.98%  (435 / 967)

  [Remaining authorable work  (normalized < 100 %)]
    Functions:  2,490
    Bytes:      1.19 MB (1,194,664 bytes)

e; counts vendor bytes in denominator |
| **Authorable fuzzy** | 79.42 % | Raw matched-code bytes over authorable-only total; best apples-to-apples byte signal |
| **Authorable normalized %** ✅ | **92.28 %** | **CANONICAL.** Functions where `match_percent_normalized == 100` over authorable total. Forgives register permutation / benign reloc-addend, but NOT wrong constants, offsets, or vtable slots |
...
| Matched code (raw bytes) | 5,042,148 bytes → **79.42 %** |
| Matched fns (fuzzy == 100) | 29,554 / 32,252 → 91.63 % |
| **Matched fns (normalized == 100)** | **29,762 / 32,252 → 92.28 %** |
| Complete units (all fns norm==100) | 435 / 967 → 44.98 % |
| Remaining fns (norm < 100) | 2,490 |
| Remaining bytes (norm < 100) | 1,194,664 bytes (1.19 MB) |
...
| Matched code (raw bytes) | 5,043,036 bytes → 44.32 % |
| Matched fns (fuzzy == 100) | 29,568 / 48,412 → 61.08 % |
| Matched fns (normalized == 100) | 29,776 / 48,412 → 61.51 % |
