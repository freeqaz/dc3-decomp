...
Ghidra provides binary analysis and decompilation for the original DC3 executable. Integrated via pyghidra-mcp for AI-assisted workflows.
...
- `--port` - now controlled by transport (default 8000 for streamable-http)
...
- PID file: `/tmp/claude/pyghidra-mcp-dc3.pid`
- Log file: `/tmp/claude/pyghidra-mcp-dc3.log`
- Project: `ghidra_projects/DC3`
...
Starting pyghidra-mcp service...
  ...
  Log: /tmp/claude/pyghidra-mcp-dc3.log
...
| Service won't start | Port 8000 already in use | Check for existing service, use `stop` first |
...
pyghidra-mcp --transport streamable-http \
    --project-name "DC3" \
    --project-directory "ghidra_projects/DC3" \
