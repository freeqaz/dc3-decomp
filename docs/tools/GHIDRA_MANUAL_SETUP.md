...
The binary is stripped. The linker map file has ~79K real symbol names.

### Quick path (headless, does everything)

```bash
./tools/ghidra/import-xex.sh
# Creates ghidra_projects/DC3/ with full analysis + all symbols applied
```

### Manual path (GUI)

Add `tools/ghidra/` to your Script Manager directories. Import `orig/373307D9/default.xex`, run analysis, then run **ImportMapFile** (Import category) and select `orig/373307D9/ham_xbox_r.map`.
...
```bash
grep "@CharBones@@" orig/373307D9/ham_xbox_r.map   # all methods in a class
grep "823486e0" orig/373307D9/ham_xbox_r.map        # symbol at address
```
