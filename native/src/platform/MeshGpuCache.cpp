...
#ifdef HX_NATIVE
        static bool sLoggedSkinDebug = false;
        const char* debugSkinning = getenv("MILO_DEBUG_SKINNING");
        if (!sLoggedSkinDebug && debugSkinning && debugSkinning[0] && debugSkinning[0] != '0') {
            const char* meshName = mesh->Name();
            if (strstr(meshName, "angel04") || strstr(meshName, "lush01_bd03")) {
                sLoggedSkinDebug = true;
                int numBones = mesh->NumBones();
                int nullBones = 0;
                for (int bi = 0; bi < numBones; ++bi) {
                    if (!mesh->BoneTransAt(bi)) {
                        nullBones++;
                    }
                }
                uint8_t maxBoneIndex = 0;
                int invalidBoneRefs = 0;
                float maxWeightSumError = 0.0f;
                for (int vi = 0; vi < unpacked; ++vi) {
                    const GpuVertexSkinned& gv = verts[vi];
                    float weightSum = gv.boneWeights[0] + gv.boneWeights[1]
                        + gv.boneWeights[2] + gv.boneWeights[3];
                    float weightSumError = fabsf(weightSum - 1.0f);
                    if (weightSumError > maxWeightSumError) {
                        maxWeightSumError = weightSumError;
                    }
                    for (int wi = 0; wi < 4; ++wi) {
                        uint8_t idx = gv.boneIndices[wi];
                        if (idx > maxBoneIndex) {
                            maxBoneIndex = idx;
                        }
                        if (idx >= numBones && gv.boneWeights[wi] > 0.0f) {
                            invalidBoneRefs++;
                        }
                    }
                }
                fprintf(
                    stderr,
                    "DC3 Native: skin-debug mesh='%s' owner='%s' compressed=%d verts=%d bones=%d"
                    " nullBones=%d maxBoneIndex=%u invalidRefs=%d maxWeightSumError=%.4f\n",
                    meshName,
                    geomOwner->Name(),
                    isCompressed ? 1 : 0,
                    unpacked,
                    numBones,
                    nullBones,
                    (unsigned)maxBoneIndex,
                    invalidBoneRefs,
                    maxWeightSumError
                );
                for (int si = 0; si < unpacked && si < 5; ++si) {
                    const GpuVertexSkinned& gv = verts[si];
                    fprintf(
                        stderr,
                        "DC3 Native: skin-sample mesh='%s' vert=%d"
                        " weights=(%.4f,%.4f,%.4f,%.4f) indices=(%u,%u,%u,%u)\n",
                        meshName,
                        si,
                        gv.boneWeights[0],
                        gv.boneWeights[1],
                        gv.boneWeights[2],
                        gv.boneWeights[3],
                        (unsigned)gv.boneIndices[0],
                        (unsigned)gv.boneIndices[1],
                        (unsigned)gv.boneIndices[2],
                        (unsigned)gv.boneIndices[3]
                    );
                }
            }
        }
