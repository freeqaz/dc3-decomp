---
name: asset-extract
description: Extract assets/materials from DC3 .ark archives and .milo/.dta files. Use when you need game assets for the native port, debugging rendering, or inspecting data.
...
Extract and inspect assets from the DC3 game archives (.ark + .milo_xbox + .dta/.dtb).
...
Extract the DC3 ark archive. This is the starting point — run it once.
...
scripts/milo/extract_ark.sh /tmp/dc3-assets --all
```

The ark header is at `orig-assets/gen/main_xbox.hdr` with 10 ark segments (0-9).
Uses `arkhelper ark2dir` from `../milo-executable-library/dance-central-3-deluxe/dependencies/linux/arkhelper`.
...
# Search by name
python3 scripts/milo/find_assets.py --grep "glitterati"
...
DC3 Xbox 360 typically uses version D (hybrid).
...
Some assets are already available without extraction:
```
~/code/milohax/milo-engine-libs/harmonix-repos/milo-rnd-library/dc3/
  config/gen/metamaterials.milo_xbox
  flow/gen/*.milo_xbox
  sfx/gen/*.milo_xbox
  ui/gen/*.milo_xbox
  world/glitterati/gen/glitterati.milo_xbox
  world/dclive/gen/dclive.milo_xbox
  char/main/gen/main.milo_xbox
```

## Tips

- The ark is ~6.2 GB across 10 segments — extraction takes a few minutes
