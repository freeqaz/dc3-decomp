...
- `run_objdiff` — build + diff a function, returns match%, verdict, enrichment. Source of truth for decomp percentages. **Always pass `project_dir`** when working in a worktree; omitting it silently measures the main repo and your edits look like they did nothing.
  - ⚠ **These tools are pinned to DC3's title ID (`373307D9`). Pointing `project_dir` at a *different repo* — e.g. `../rb3-xenon` — does NOT measure that repo. It silently measures DC3 and returns a DC3 number.** Measured 2026-08-04: a cross-repo call returned 93.3% (DC3's baseline for that function) where the rb3-xenon value was 92.2% — close enough to be mistaken for a real delta and written into a commit message. When working in another decomp tree, use **that tree's own** objdiff tooling and validate it against that tree's `report.json` before trusting any number.
