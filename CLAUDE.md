...
Use the `mcp__orchestrator__` tools for all decomp analysis. **Reach for `bin/objdiff-cli` only for the cases named below** — and if you find another, that is a tooling gap worth closing, not a rule worth quietly breaking. (The old blanket "do not call `objdiff-cli` directly" was unfollowable: a sweep of 474 session transcripts found 296 real invocations, 259 of them against DC3, led by `--include-data` (88) and `--batch` (49) — capabilities the wrappers did not have. Gaps closed 2026-08-19; the honest exceptions are named rather than denied. Full table: [docs/tools/REFERENCE.md](docs/tools/REFERENCE.md#objdiff-cli-through-mcp-what-maps-to-what).)
