...
- `run_objdiff` — build + diff a function, returns match%, verdict, enrichment. Source of truth for decomp percentages. **Always pass `project_dir`** when working in a worktree; omitting it silently measures the main repo and your edits look like they did nothing.
  ...
  - `diff_mode: "raw"` (= `functionRelocDiffs=all`) counts relocations. Normalized scoring — the fork's **default**, so omitting `-c` is *not* raw — discards exactly the plane where wrong-vtable-slot and wrong-callee bugs live; `Splash::Suspend`/`Resume` read 100.0% normalized while dispatching each other's virtual. ⚠ `run_diff_inspect(diff_mode="raw")` was **inert** until 2026-08-19 (it omitted the flag), so any "no relocation divergence" conclusion drawn from it before then is unsupported.
