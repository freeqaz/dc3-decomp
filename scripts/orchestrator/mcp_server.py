...
def _stack_signal_summary(instrs: list, data: "dict | None" = None) -> "str | None":
    ...
    frame_known = (getattr(tgt_prol, "frame_known", True)
                   and getattr(base_prol, "frame_known", True))
    ...
    if swapped > 0:
        ...
        hint = (" — same slot SET, variables assigned differently "
                "(slot-allocation shaping)")
        ...
        async def list_tools() -> list[Tool]:
            return [
                ...
                Tool(
                    ...
                    description="Deep analysis of WHY a function doesn't match. Provides root cause diagnosis, cluster analysis, register swap detection, offset analysis, replace categorization, before/after comparison, and stack-layout slot diff (with base-side variable names from a /Z7 CodeView recompile). Use after run_objdiff when you need deeper insight. For functions with stack signal flagged in run_objdiff output, use mode=stack-layout.\n\n⚠️ CRITICAL: Pass project_dir parameter when in a worktree!",
                    inputSchema={
                        "type": "object",
                        "properties": {
                            "symbol": {
