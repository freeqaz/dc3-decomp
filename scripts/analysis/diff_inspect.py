...
def _extract_frame_size_from_instrs(instrs, side='target', prologue_info=None):
    ...
    instructions on this side to scan at all". Both callers of this function
    ...
    `instructions` array, and a target-only function returns rows whose `base`
    side is absent on every row.
    ...
    invents a 0. Returning None for "no evidence" is what the two
    `TestExtractFrameSize` cases named `*_returns_none_*` have always asked
...
def cmd_stack_layout(instrs, symbol=None, project_dir=None, data=None):
    ...
    *data* is the full objdiff JSON dict.  When provided and it contains a
    ...
    #   (16.3% of this binary's functions are genuinely frameless), and, once
