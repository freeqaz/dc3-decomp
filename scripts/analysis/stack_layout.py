...
def frame_base_regs(instrs: list, side_key: str) -> set:
    ...
    ★ r31 is a frame base ONLY when the prologue derives it from r1. Otherwise
    it holds an incoming object pointer, and `lwz r3, 0x50, r31` is then a CLASS
    MEMBER load, not a stack slot.

    MEASURED IN THIS REPO (373307D9), 2026-08-04, over the 2,236 partial-match
    functions that have a base .obj: the target-side prologue derives r31 from r1
    in 1,186 of them (53.0%) and does NOT in the other 1,050 (47.0%).

    v1 hardcoded {"r1", "r31"} unconditionally. Measured consequence on that same
    corpus: 682 functions (30.5%) had class-member accesses tabulated as stack
    slots -- 3,515 phantom slots -- i.e. the stack-layout table was silently
    mixing class layout into a stack report.

    Same 20-instruction prologue window and same derivation rule as
    rb3-xenon's tools/r31_role_census.py. That tool has no counterpart in this
    repo, so the rule is ported but its 4 hard-asserted positives are not
    re-verifiable here; the selftest fixtures 6/6b are the local guard.
    """
...
@dataclass
class Prologue:
    # ★ TRI-STATE, and the whole point of the 2026-08-04 repair (back-ported
    ...
    #           found NO frame allocation (frameless leaf). 16.3% of this
    #           binary — the measured survey below — so 0 is a real answer,
    #           not a nobody-home default.
...
# ── Frame-size scanning ──────────────────────────────────────────────────────
#
# MEASURED IN THIS REPO (373307D9), 2026-08-04 -- NOT inherited from rb3-xenon,
# whose survey describes a different binary. 69,386 functions in 2,223 fresh
# target .s files under build/373307D9/asm (8 .s files predating the 2026-07-12
# symbols.txt regeneration were skipped as stale generations):
#
#     STWU   58,002  83.59%   stwu  r1, -N, r1            <- v1 handled only this
#     NONE   11,377  16.40%   no frame allocation (leaf)
#     STWUX       7   0.01%   lis/ori rX + stwux r1,r1,rX <- v1 read 0 here
#
# (Including the 8 stale files instead gives 69,436 / 83.57% / 16.42% / 0.01%:
# the filter is NOT load-bearing here, unlike in rb3-xenon where two .s files
# genuinely disagreed with symbols.txt. Stated so nobody re-derives it.)
#
# So the *coverage* gap is 7 functions. The *representation* bug (0 == unparsed)
# is the one that matters, because it is unbounded: any future unmodelled form
# silently reads 0 and prints "frames match".
#
# Independent corroboration channel: MSVC also aliases r31 to the new r1 via
# `subi r31, r1, N` before the stwu, so where both channels exist a disagreement
# is a real anomaly and is reported rather than silently resolved.
...
def _scan_frame_size(instrs: list, side_key: str, horizon: int) -> tuple:
    ...
    ★ 2026-08-17. `scanned` — the count of instructions actually present on
      ...
      it is the same defect as the `saw_alloc_form` one above, one layer down.
      ...
      then read True and every consumer downstream — mcp_server's `frame Δ`
      line in every run_objdiff response, print_report's refusal, cmd_stack_
      layout's delta — treated the fabricated 0 as a measurement.

      This is reachable from live objdiff-cli 4.2.3, not just from tests:
      `objdiff-cli diff -p . __savegprlr --include-instructions -f json`
      returns a well-formed diff whose `instructions` array is EMPTY, and
      `?PreInit@Synth360@@UAAXXZ` returns 345 rows of which exactly 1 carries
      a `base` side. Absence of a side is a routine objdiff shape.

      0 is still a real, common answer and is NOT weakened here: 15.86% of
      this binary's 69,307 functions have neither `mflr r12` nor any
      allocation, and a further 0.44% (306 functions, e.g.
      `?GetBuffer@JsonWriter@@QAAJPADPAK@Z` — `mflr r12; bl __savegprlr_29`
      and no `stwu`) save registers into the caller's frame without
      allocating one. Seeing instructions and finding no allocation means 0.
      Seeing NO instructions means None. The difference is whether we looked.
    """
...
def parse_prologue(instrs: list, side_key: str) -> Prologue:
    ...
    for ins in instrs[:horizon]:
        ...
        #
        #     TGT: bl __savegprlr     BASE: bl __savegprlr_14
        #
        #   config/373307D9/symbols.txt settles it as ground truth, not inference
        #   (MEASURED IN THIS REPO, addresses are 373307D9's, not xenon's):
        #     __savegprlr    = .text:0x8299D8F0; // type:function size:0x50
        #     __savegprlr_14 = .text:0x8299D8F0; // type:label
        #     __savegprlr_15 = .text:0x8299D8F4; // type:label
        #   i.e. bare == _14 (the helpers ascend 4 bytes per entry, saving one
        #   fewer register each). v1's regex REQUIRED the _NN, so 479 bare
        #   __savegprlr + 26 bare __savefpr call sites in this binary silently
        #   scored 0 saved registers, fabricating a GPR delta out of pure symbol
        #   naming. Once fixed both sides read the same count.
        if op == "bl":
...
#
# Every fixture is built IN MEMORY: no
...
def _bigframe_prologue(hi, lo, idx0=0):
    """The big-frame shape: lis/ori + subf r31 + lis/ori + stwux.
    MEASURED present in this binary (e.g. BinkMovieImpl::PlatformCacheFile,
    system/oggvorbis/psy.s, lib/binkxenon/expand.s)."""
...
def selftest():
    ...
    # could see the empty-input branch. Every case below is reachable from
    # live objdiff-cli 4.2.3 output, not just from a unit test.
    ...
    # `objdiff-cli diff -p . __savegprlr` really does return an empty
    # `instructions` array, and `?PreInit@Synth360@@UAAXXZ` really does return
    # 345 rows with exactly one `base` side. This is the >1-row shape of that:
    # rows exist, but not one of them carries the side we were asked about.
    ...
    # and 16.3% of this binary is genuinely frameless — 0 must survive.
    # ?PreInit@Synth360@@UAAXXZ's base side is exactly this: one `blr`.
    stub = [_ins(0, ("mflr", "r12"), ("blr", ""))]
    check("a ONE-instruction stub side is KNOWN-zero, not UNKNOWN",
          _scan_frame_size(stub, "base", H)[0], 0)
    # ?GetBuffer@JsonWriter@@QAAJPADPAK@Z: mflr + save helper, no allocation.
    # 306 functions in this binary. Must NOT be dragged into UNKNOWN.
