...
starts in**, because ``setup_worktree.sh`` reflinks main's ``build/373307D9/``.
...
def _ninja_targets(project_dir):
    ...
    for line in out.splitlines():
        ...
        if tgt.endswith(".obj") and "/373307D9/src/" in tgt:
            objs[tgt] = True
    return list(objs)


def _obj_for_source(project_dir, src_rel):
    """build/373307D9/<src path>.obj, if build.ninja knows it."""
    cand = os.path.join("build", "373307D9", os.path.splitext(src_rel)[0] + ".obj")
...
def build_command(project_dir, obj, cls, scratch_obj, use_pch=False, all_classes=False,
    ...
    # ---- DC3 PORT.  This repo's compile edge is a SHELL PIPELINE, not a bare
    # argv:  `cd <srcdir> && <wibo> ... cl.exe ... <file>.cpp && python3
    # scripts/obj_build_metadata_patcher.py --obj ...`.  Two consequences:
    #   * the source operand is RELATIVE to the `cd` directory, so the layout
    #     compile has to run from there, not from project_dir; and
    #   * the trailing `&&` stage rewrites the object's clock fields.  We must
    #     drop it -- it would run against /Fo's scratch object, and it is not
    #     part of the compile whose layout we are asking about.
    # Returning the cwd alongside argv/env keeps this repo-specific shape in
    ...
    if tu:
        ...
        if not obj:
            raise SystemExit(f"{tu} has no compile edge in build.ninja "
                             f"(add it to config/373307D9/objects.json first)")
        src = tu
    else:
        if not cls:
...
def audit_header(project_dir, header, cls_info, cls=None):
    ...
    # DERIVED class's body, so it is about the derived member.  Measured on DC3
    # 2026-09-01: `Spotlight::mTarget // 0x2dc` is CORRECT (compiler agrees),
    # but `RndTransformable::mTarget` at 0xe4 came first and the row was
    # reported as wrong by -504.  `--fix-header` writes these rows back, so
    # this would have REWRITTEN A CORRECT COMMENT TO A WRONG VALUE -- the same
    # corruption shape the cross-class scoping fix above exists to prevent.
    # 47 of 176 raw rows on this tree were this family alone.
    real = {}
...
def run_selftest(project_dir, sabotage=None, verbose=False):
    ...
    #       setup_worktree.sh reflinks main's build/373307D9/.  A local full
