...
def cmd_permute(args):
    ...
    if not resolved:
        print(f"Error: Could not resolve '{args.symbol}' from decomp.db")
        sys.exit(1)

    mangled, source_path, qualified_name = resolved
    if not args.quiet:
        print(f"Permuting: {qualified_name}")
    ...
    # Enable BSF-guided mode on declaration_reorder
    for p in patterns:
    ...
    result = hill_climb(
        ...
        ghidra=not args.no_ghidra,
        chain=True,
        adaptive=True,
        constrained=not args.no_ghidra,
    )

    # Extract best pattern from improving rounds
    best_pattern = None
    ...
    if args.json:
        ...
        delta = result.final_percent - result.initial_percent
        print(f"\nResult: {result.initial_percent:.2f}% → {result.final_percent:.2f}% ({'+' if delta >= 0 else ''}{delta:.2f}%)")
        print(f"Rounds: {len(result.rounds)}, Stopped: {result.stopped_reason}")
        if best_pattern:
            print(f"Best pattern: {best_pattern}")
        if result.final_percent >= 100.0:
            print("COMPLETE!")

    # Report to orchestrator DB if improvement was made
    if result.final_percent > result.initial_percent:
        try:
            func = get_function_by_symbol(mangled, db_path=args.db)
            if func:
                status = "complete" if result.final_percent >= 100.0 else "at_limit"
                record_attempt(
                    function_id=func["id"],
                    session_id=f"permuter-{int(time.time())}",
                    model="permuter",
                    start_percent=result.initial_percent,
                    end_percent=result.final_percent,
                    exit_status=status,
                    notes=f"hill_climb: {result.stopped_reason}, best_pattern={best_pattern}",
                    db_path=args.db,
                )
                update_function_status(
                    func["id"],
                    current_percent=result.final_percent,
                    db_path=args.db,
                )
...
def main():
  ...
  ./bin/orchestrate targets --json --limit 100"""
    ...
    p_permute = subparsers.add_parser(
        "permute",
        help="Run source permuter (automated pattern search)",
        description="Run the hill-climbing source permuter on a function. Tries AST-level transformations (declaration reorder, comparison flip, ternary swap, etc.) to find matching code.",
        epilog="""Examples:
  # Permute a single function
  ./bin/orchestrate permute '?TriggerSelf@EventTrigger@@IAAXXZ'

  # Permute with specific patterns only
  ./bin/orchestrate permute '?TriggerSelf@EventTrigger@@IAAXXZ' --patterns declaration_reorder,comparison_flip

  # Dry run with JSON output
  ./bin/orchestrate permute '?Poll@CharMirror@@UAEXXZ' --no-apply --json"""
    )
    p_permute.add_argument("symbol", help="Mangled function symbol")
    p_permute.add_argument("--patterns", default="all", help="Comma-separated patterns or 'all' (default: all)")
    p_permute.add_argument("--max-rounds", type=int, default=10, help="Max hill-climbing rounds (default: 10)")
    p_permute.add_argument("--max-variants", type=int, default=100, help="Max variants per round (default: 100)")
    p_permute.add_argument("--plateau-limit", type=int, default=3, help="Stop after N rounds without improvement (default: 3)")
    p_permute.add_argument("--workers", type=int, default=6, help="Parallel scoring workers (default: 6)")
    p_permute.add_argument("--no-apply", action="store_true", help="Don't apply best result to source")
    p_permute.add_argument("--no-ghidra", action="store_true", help="Disable Ghidra-guided patterns")
    p_permute.add_argument("--quiet", "-q", action="store_true", help="Suppress output")
    p_permute.add_argument("--json", action="store_true", help="Output as JSON")
