...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--no-report-build", action="store_true",
        help="Skip regenerating report.json before the DB sync (by default the "
             "report is rebuilt via tools/ninja-locked so wins landed by other "
             "agents since the last build are picked up, not just stale ones)",
    )
    return parser.parse_args()


def regenerate_report(timeout: int = 1800) -> bool:
    """Rebuild report.json via the locked ninja wrapper so the DB sync below
    ingests *fresh* percentages.

    The startup sync (sync_db_from_report) only ever sees what's on disk. If
    other agents/sweeps landed wins since the last full build, an unchanged
    report.json still lists those functions as workable and we waste a sweep
    re-attacking them. Regenerating here closes the build->sweep loop. ninja
    is incremental and the wrapper flock-serializes, so concurrent sweeps are
    safe. Degrades gracefully: on any failure we fall back to the existing
    report rather than aborting the sweep.
    """
    ninja_locked = _project.repo_root / "tools" / "ninja-locked"
    if not ninja_locked.exists():
        print(f"  (skipping report build — no {ninja_locked})", file=sys.stderr)
        return False
    target = f"{_project.build_prefix}/report.json"
    print(f"  Rebuilding {target} (tools/ninja-locked)...", file=sys.stderr)
    try:
        r = subprocess.run(
            [str(ninja_locked), target],
            cwd=str(_project.repo_root),
            capture_output=True, text=True, timeout=timeout,
        )
    except subprocess.TimeoutExpired:
        print("  (report build timed out, using existing report)", file=sys.stderr)
        return False
    if r.returncode != 0:
        tail = (r.stderr or r.stdout or "").strip().splitlines()[-3:]
        print(f"  (report build failed rc={r.returncode}, using existing report) "
              f"{' / '.join(tail)}", file=sys.stderr)
        return False
    return True


def sync_db_from_report(rebuild: bool = True) -> None:
    ...
    When `rebuild` is set we first regenerate report.json so the ingest reflects
    the current source tree (including wins from concurrent agents), not a report
...
def main():
    ...
    if not args.no_db_sync:
        ...
        sync_db_from_report(rebuild=not args.no_report_build)
