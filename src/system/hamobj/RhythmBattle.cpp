...
void SetJump(int x, int y) {
    ...
    int by = y - 1;
    if (bx == by) {
        TheMaster->GetAudio()->SetLoop(by * 4, bx * 4);
    } else {
        float start = BeatToMs(bx * 4);
        float end = BeatToMs(by * 4);
        float crossfade_beats = SystemConfig("synth", "crossfade_beats")->Float(1);
        float crossfade_end = BeatToMs(bx * 4 + crossfade_beats);
...
void RhythmBattle::OnBeat() {
    ...
    if (onMeasure) {
        ...
        if (mFullKTB && !mFinale) {
            if (!b22) {
                ...
                bool zoneActive =
                    !mJackCooldown && (mPlayerOne->InTheZone() || mPlayerTwo->InTheZone());
                float f48 = 0;
    ...
    if ((onMeasure && !mBattleFinished) || b22) {
        float windowStart = beat;
        float windowEnd = windowStart + 4.0f;
        mPlayerOne->SetWindow(windowStart, windowEnd);
        mPlayerTwo->SetWindow(windowStart, windowEnd);
    }
    if (mBattleFinished) {
        End();
    }
            ...
            if (!mFinaleSequenceTimer) {
                ...
                if (mFinalePhaseIndex == 1) {
                    ...
                    if (mFinale) {
                        ...
                        UIPanel *panel = TheUI->FocusPanel();
                        panel->SetProperty(tan_destroyed, true);
