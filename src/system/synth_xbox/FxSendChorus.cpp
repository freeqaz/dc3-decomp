...
// Reconstructed from the target. The chorus reuses FlangerEffect, but where
// FxSendFlanger360 hands the depth straight through, the chorus converts its
// centitone depth into the flanger's percentage: it asks how far one LFO
// period of pitch modulation moves the playback speed, turns that into a
// transposition, and expresses mDepth as a percentage of it.
void FxSendChorus360::SyncEffectParams(IXAudio2SubmixVoice *voice) const {
    ...
    p.mRate = rate;
    float transpose = CalcTransposeFromSpeed(rate * mDelayMs * 0.0062831854f + 1.0f);
    p.mFeedback = mFeedbackPct;
    p.unk0 = mBypass;
    p.mWet = mOffsetPct;
    p.mDepth = Clamp(0.0f, 100.0f, mDepth / (transpose * 100.0f) * 100.0f);
