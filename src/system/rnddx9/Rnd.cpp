...
void DxRnd::DrawRect(
    ...
    // statements per corner add 7 instructions and cost ~3pp.
    if (mat && !mat->Prelit()) {
    ...
    // material's texture transform.  Written out per corner rather than
    // through a helper: the target inlines the corner constants, and MSVC
    // folds a multiply by 1.0f but NOT one by 0.0f (x*0.0f is not x for
    // NaN/Inf), so the 0.0f factors have to survive into the source.
    if (texXfm) {
    ...
    // callee-saved register across the Matrix4 temporary's constructor, which
    // is what the target does; without it MSVC reloads the global afterwards
    // and the whole callee-saved allocation shifts (99.5% -> 100.0%).  It has
    // to be declared HERE, not at the top of the function: hoisting it costs
    // an extra register and drops the function to 98.5%.
