...
void RndVelocityBuffer::DrawMesh(RndMesh *mesh) const {
    ...
    if (mat != nullptr && mat->GetZMode() != kZModeTransparent) {
        ...
        // NumBones() is called ONCE (0x826B1FB4 `subf` / 0x826B1FE0 `divw`) and
        // the raw value is spilled to 0x60(r1) at 0x826B1FEC because the notify
        // below takes it by const reference.  The clamp is `Max(1, n)`:
        // 0x826B1F94 `li r29, 0x1` (shared with the mShouldCache store two
        // instructions later), 0x826B2000 `cmpwi cr6, r9, 0x1`, `ble` keeps the
        // 1, `mr r29, r9` otherwise.  `if (n <= 0) n = 1;` compares against 0.
        int rawBones = mesh->NumBones();
        int numBones = Max(1, rawBones);

        // Both cache keys and both cache element addresses are materialised
        // BEFORE the first GetXfms -- 0x826B1FF0 `lwzx r5, r7, r31` and
        // 0x826B1FF8 `lwzx r26, r6, r31` for the keys, 0x826B1FF4
        // `addi r3, r11, 0xac` and 0x826B1FFC `addi r8, r10, 0xac` for the two
        // RndXfmCache pointers (element stride 0x1b584, base 0xac).
